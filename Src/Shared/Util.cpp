...
SERVER_DECL const char* _StringToUTF8(const char*   pASCIIBuf)
{
...
#else
	memset(_StringConversionStorage,0,2048);
	size_t intlen = strlen(pASCIIBuf);
	size_t outlen = 2048;
	char* in = (char*)pASCIIBuf;
	char* out = _StringConversionStorage;
	...
	size_t ret = -1;
	ret = iconv(cd,&in,&intlen,&out,&outlen);
	if (ret == (size_t)(-1))
	{
		return &pASCIIBuf[0];
	}
...
#else
	memset(_StringConversionStorage,0,2048);
	size_t intlen = strlen(pUtf8Buf);
	size_t outlen = 2048;
	char* in = (char*)pUtf8Buf;
	char* out = _StringConversionStorage;
...
namespace Au
{
	...
	void Au_Sleep(unsigned long timems)
	{
#ifdef WIN32
		::Sleep(timems);
#else
