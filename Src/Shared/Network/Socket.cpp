...
Socket::Socket(SOCKET fd, uint32 sendbuffersize, uint32 recvbuffersize) : m_fd(fd), m_connected(false),	m_deleted(false),m_isOk(false), m_writeLock(0)
{
	// Allocate Buffers
...
void Socket::setSocketOK()
{
	m_isOk.SetVal(true);
}

void Socket::setSocketNotOK()
{
	m_isOk.SetVal(false);
}
