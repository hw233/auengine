...
void oLog::outLuaString(const char* str, ...)
{
#ifndef WIN32
	return;
#endif
...
void oLog::Init(int32 fileLogLevel, std::string namestr)
{
	SetFileLoggingLevel(fileLogLevel);

	std::string logNormalFilename,logErrorFilename;
	logNormalFilename = namestr + "-normal.log";
	logErrorFilename = namestr + "-error.log";
	
	m_normalFile = fopen(logNormalFilename.c_str(), "w");
	if(m_normalFile == NULL)
	...
	m_errorFile = fopen(logErrorFilename.c_str(), "w");
	if(m_errorFile == NULL)
