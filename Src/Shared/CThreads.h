...
class SERVER_DECL CThread : public ThreadBase
{
	public:
		...
		AU_INLINE void SetThreadState(CThreadState thread_state) { ThreadState.SetVal(thread_state); }
		AU_INLINE CThreadState GetThreadState()
		{
	...
	protected:
		...
		Au::Threading::AtomicCounter ThreadState;
		time_t start_time;
		int ThreadId;
};
