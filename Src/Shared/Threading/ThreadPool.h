...
class ThreadController
{
	...
	public:
		...
		AU_INLINE uint32 GetId() { return (uint32)thread_id; }
};
...
class SERVER_DECL CThreadPool
{
	...
	public:
		...
		AU_INLINE void Gobble() { _threadsEaten = (int32)m_freeThreads.size(); }

		// gets active thread count
		AU_INLINE uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size(); }

		// gets free thread count
		AU_INLINE uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};
