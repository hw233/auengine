...
#ifdef WIN32
#define AU_FORCEINLINE __forceinline
#else
#define AU_FORCEINLINE inline
#endif
...
AU_INLINE void reverse_array(uint8* pointer, size_t count)
{
	size_t x;
...
AU_INLINE void au_TOLOWER(std::string & str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
}

AU_INLINE void au_TOUPPER(std::string & str)
{
	for(size_t i = 0; i < str.length(); ++i)
