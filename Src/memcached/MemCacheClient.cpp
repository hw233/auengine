...
class MemCacheClient::Server : public Mem::Socket
{
public:
    /*! @brief constructor */
    ...
    ) 
{
    if (this != &rhs) {
        mTrace = rhs.mTrace;
        strncpy(mAddress, rhs.mAddress, sizeof(mAddress));
        mIp   = rhs.mIp;
        mPort = rhs.mPort;
        mLastConnect = 0;
    }
    ...
    ) 
{
	...
	Server::ConnectResult rc = this->Connect(1000, 300*1000);
	switch (rc){
...
MemCacheClient::DumpTables()
{
    ...
    for (size_t n = 0; n < mServerHash.size(); ++n) {
        ...
        snprintf(buf, sizeof(buf), "%s>%d>%x>%x>", server.mServer->GetAddress(), 
    ...
    )
{
    ...
    while (nItem < nItemCount) {
        ...
        catch (const Mem::Socket::Exception & e) {
    ...
    )
{
    ...
    for (;;) {
        ...
        if (0 != strncmp(sValue.data(), "VALUE ", 6)) {
            throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at VALUE");
        }

        // extract the key
        int n = (int) sValue.find(' ', 6);
        if (n < 1) throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at key");
        std::string sKey(sValue, 6, n - 6);

        // extract the flags
        const char * pVal = sValue.data() + n + 1;
        unsigned nFlags = (unsigned) strtoul(pVal, (char**) &pVal, 10);
        if (*pVal++ != ' ') throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at flags");

        // extract the size
        unsigned nBytes = (unsigned) strtoul(pVal, (char**) &pVal, 10);
        if (*pVal != ' ' && *pVal != '\r') throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at size");
        ...
        if (*pVal == ' ') {
            ...
            if (*last != '\r') throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at CAS");
        }
        ...
        if ('\r' != aServer->GetByte() ||
            '\n' != aServer->GetByte())
        {
            throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad get response at trail");
        }
    ...
    )
{
    ...
    for (MemRequest ** p = aBegin; p < aEnd; ++p) {
        ...
        aServer->disconnect();
        throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad del response");
    }
    ...
    )
{
    ...
    aServer->disconnect();
    throw Mem::Socket::Exception(Mem::Socket::ERR_OTHER, 0, "bad store response");
}
    ...
    )
{
    ...
    for (size_t n = 0; n < mServer.size(); ++n) {
        ...
        catch (const Mem::Socket::Exception &) {
